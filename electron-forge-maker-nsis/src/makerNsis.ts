...
import { getAppUpdateYml, getChannelYml } from "electron-updater-yaml";
import fs from "fs-extra";
import path from "path";

import type { MakerNSISConfig } from "./config";
...
export default class MakerNSIS extends MakerBase<MakerNSISConfig> {
	...
	async codesign(options: MakerOptions, outPath: string) {
		if (this.config.codesign) {
			...
			if (!process.env.CSC_LINK && this.config.codesign.certificateFile) {
				log(
					`Setting process.env.CSC_LINK to ${this.config.codesign.certificateFile}`,
				);
			...
			if (
				...
				this.config.codesign.certificatePassword
			) {
	...
	async createAppUpdateYml(options: MakerOptions, outPath: string) {
		...
		await fs.writeFile(
			path.join(outPath, "resources", "app-update.yml"),
			ymlContents,
			"utf8",
		);
	}
